package com.example.signalrtestandroid.activities;
...
import com.example.signalrtestandroid.Events.chatEvents.FileResendEvent;
...
import com.example.signalrtestandroid.baseClasses.BaseActivity;
import com.example.signalrtestandroid.commons.ConnectionService;
...
import com.example.signalrtestandroid.commons.SignalRHelper;
...
import com.example.signalrtestandroid.fragments.ConversationsFragment;
...
import com.example.signalrtestandroid.model.chat.BulkConversationModel;
...
import com.example.signalrtestandroid.model.chat.OrganizationModelData;
import com.example.signalrtestandroid.model.chat.QueueConversationModel;
                ...
                try {
                    ...
                    if(e.getMessage().equalsIgnoreCase("The 'invoke' method cannot be called if the connection is not active.") || e.getMessage().equalsIgnoreCase("The 'send' method cannot be called if the connection is not active")|| e.getMessage().contains("is not active")){
                        if (hubConnection != null && signalRHelper != null) {
                            boolean isSignalRConnect =  signalRHelper.startSignalRHubClient(hubConnection, common.getCustomerID(mContext),common.getDeviceToken(mContext),mContext, MainActivityChat.this);
                            if(isSignalRConnect){
